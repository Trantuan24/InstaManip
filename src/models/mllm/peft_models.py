...
def get_peft_model_with_resize_embedding(model, peft_config=None, model_id=None, vocab_size=None, torch_dtype='bf16'):
    ...
    if isinstance(model, DictConfig):  # False
    ...
    if vocab_size is not None:
        ...
        if vocab_size - old_vocab_size > 0:  # = 0
            with deepspeed.zero.GatheredParameters(list(model.get_input_embeddings().parameters()) + list(model.get_output_embeddings().parameters()), modifier_rank=0):
                
                input_embeddings = model.get_input_embeddings().weight.data
                input_embeddings_avg = input_embeddings[:-vocab_size + old_vocab_size].mean(dim=0, keepdim=True)
                ...
                if model.get_output_embeddings() is not None and not model.config.tie_word_embeddings:
                    ...
                    output_embeddings_avg = output_embeddings[:-vocab_size + old_vocab_size].mean(dim=0, keepdim=True) * 3
                    output_embeddings[-vocab_size + old_vocab_size:] = output_embeddings_avg

    if peft_config is not None:
        print('peft config: ', peft_config)
        if isinstance(peft_config, DictConfig):  # False
