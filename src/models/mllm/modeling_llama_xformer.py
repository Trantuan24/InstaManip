...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached:  # False
...
class LlamaAttention(nn.Module):
    ...
    def group_self_attention(self, query, key, value, attn_bias, p=0.0, training=True, ids_exemplar_source_mask=None, ids_exemplar_target_mask=None,
        ...
        key = key.transpose(1, 2)
        value = value.transpose(1, 2)
        attn = query @ key.transpose(-2, -1)  # (B, head num, L, L)
        if attn_bias is not None:
            attn = attn + attn_bias
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    ...
                    def custom_forward(*inputs):
                        return module(*inputs, output_attentions, False)  # False for use_cache
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        return CausalLMOutputWithPast(
            ...
            hidden_states=outputs.hidden_states,  # input embeds + output of 40 layers
